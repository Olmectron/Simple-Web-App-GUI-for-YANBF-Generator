define(["exports","./my-app.js"],function(_exports,_myApp){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.PaperSpinnerBehavior=_exports.$paperSpinnerBehavior=void 0;(0,_myApp.Polymer)({_template:_myApp.html`
    <style>
      :host {
        display: block;
    ...
    <slot></slot>
`,is:"iron-collapse",behaviors:[_myApp.IronResizableBehavior],properties:{/**
...
</iron-iconset-svg>`;document.head.appendChild(template.content);const template$1=_myApp.html`
...
`;template$1.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 */(0,_myApp.Polymer)({_template:template$1,is:"paper-fab",behaviors:[_myApp.PaperButtonBehavior],properties:{/**
     ...
     */label:{type:String,observer:"_labelChanged"}},_labelChanged:function(){this.setAttribute("aria-label",this.label)},_computeIsIconFab:function(icon,src){return 0<icon.length||0<src.length}});const PaperSpinnerBehavior={properties:{/**
...
</dom-module>`;document.head.appendChild($_documentContainer.content);const template$2=_myApp.html`
...
`;template$2.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 @hero hero.svg
                                                 @demo demo/index.html
                                                 */(0,_myApp.Polymer)({_template:template$2,is:"paper-spinner",behaviors:[PaperSpinnerBehavior]});class GameItem extends _myApp.PolymerElement{static get template(){return _myApp.html$1`
      <style >
        :host {
          display: block;
...
this.push("ndsFiles",{file:files[i]})}}_getCurrentPath(){const{remote}=require("electron");if(null!=this.directoryPath&&""!=this.directoryPath.trim()){return this.directoryPath.trim()}var camino=remote.process.env.PORTABLE_EXECUTABLE_DIR;if(!camino){camino="."}return camino}callPythonGenerator(template){var context=this,ndsFile=template.lastNdsFile;console.log("NDS FILE",ndsFile);const spawn=require("child_process").spawn;var camino=ndsFile.file.path,outputName="YANBF-"+ndsFile.file.name.substring(0,ndsFile.file.name.length-4).replace(/ /g,"_")+".cia",outputLog="YANBF-"+ndsFile.file.name.substring(0,ndsFile.file.name.length-4).replace(/ /g,"_")+".log";console.log("Output name",outputName);var arrayArgs=[];arrayArgs.push(camino);arrayArgs.push("-o");arrayArgs.push(context._getCurrentPath()+"/output/"+outputName);if(null!=ndsFile.boxArtFile){arrayArgs.push("-b");arrayArgs.push(ndsFile.boxArtFile.path);console.log("Path File",ndsFile.boxArtFile.path)}console.log("Array args",arrayArgs);const pythonProcess=spawn(context._getCurrentPath()+"/generator", arrayArgs,{cwd:this._getCurrentPath()});template.set("lastNdsFile.running",!0);template.set("lastNdsFile.finished",!1);template.set("lastNdsFile.hadSuccess",!1);return new Promise(function(resolve,reject){/*    pythonProcess.on("error",function(err){
            console.log("Python error",err);
          });
          pythonProcess.on("close",function(err){
            console.log("Python close",err);
          });*/pythonProcess.stderr.on("data",data=>{var result=data.toString();console.log("Python error",result);const fs=require("fs");var now=new Date;fs.writeFile(context._getCurrentPath()+"/output/logs/"+outputLog,now.toString()+": "+result+"\r\n",{flag:"a+"},function(err){if(err)return console.log(err);//console.log('Hello World > helloworld.txt');
template.set("lastNdsFile.lastErrorLog",context._getCurrentPath()+"/output/logs/"+outputLog)})});pythonProcess.stdout.on("data",data=>{var result=data.toString();const fs=require("fs");var now=new Date;fs.writeFile(context._getCurrentPath()+"/output/logs/"+outputLog,now.toString()+": "+result+"\r\n",{flag:"a+"},function(err){if(err)return console.log(err);//console.log('Hello World > helloworld.txt');
template.set("lastNdsFile.lastErrorLog",context._getCurrentPath()+"/output/logs/"+outputLog)});console.log("Python print",result);template.set("lastNdsFile.running",!1);template.set("lastNdsFile.finished",!0);if(null!=result&&-1<result.indexOf("CIA generated.")){console.log("Success!");template.set("lastNdsFile.hadSuccess",!0);var path=require("path"),ciaPath=path.resolve(context._getCurrentPath()+"/output/"+outputName),outputPath=path.resolve(context._getCurrentPath()+"/output");template.set("lastNdsFile.lastSuccessCia",ciaPath);template.set("lastNdsFile.outputPath",outputPath);resolve({ndsFile:ndsFile,success:!0})}else{console.log("Failure!");template.set("lastNdsFile.hadSuccess",!1);resolve({ndsFile:ndsFile,success:!1})}})})}downloadAllTemplates(){var fs=require("fs"),dir=this._getCurrentPath()+"/output";if(!fs.existsSync(dir)){fs.mkdirSync(dir)}if(!fs.existsSync(dir+"/logs")){fs.mkdirSync(dir+"/logs")}for(var context=this,templates=this.shadowRoot.querySelectorAll("game-item"),templateArray=[],i=0;i<templates.length;i++){templateArray.push(templates[i]);//      arr.push(this.downloadTemplate(templates[i]));
}if(!templateArray.length){MiscUtils.Toast.show("You didn't add any files");return}context.set("saving",!0);/*var arr=[];
                                 for(var i=0;i<templates.length;i++){
                                       arr.push(this.callPythonGenerator(templates[i]));
                                 //      arr.push(this.downloadTemplate(templates[i]));
                                 }*/const{shell}=require("electron");var path=require("path"),outputPath=path.resolve(this._getCurrentPath()+"/output");return templateArray.reduce((p,template)=>{return p.then(()=>{return context.callPythonGenerator(template);//function returns a promise
})},Promise.resolve()).then(result=>{console.log("Result",result);context.set("saving",!1);MiscUtils.Toast.show("The generation process has ended");shell.openPath(outputPath)}).catch(function(err){context.set("saving",!1);console.error("Errors!",err);MiscUtils.Toast.show("The generation process has ended with errors");shell.openPath(outputPath)})}ready(){super.ready();var context=this;setTimeout(function(){context.setupDropZone()},500)}setupDropZone(){var dropZone=this.shadowRoot.querySelector("#dropZone"),context=this;// Optional.   Show the copy icon when dragging over.  Seems to only work for chrome.
dropZone.addEventListener("dragover",function(e){e.stopPropagation();e.preventDefault();e.dataTransfer.dropEffect="copy"});// Get file data on drop
dropZone.addEventListener("drop",function(e){e.stopPropagation();e.preventDefault();// Array of all files
for(var files=e.dataTransfer.files,i=0,file;file=files[i];i++){console.log("FILEddd",file);if(file.name.endsWith(".nds")||file.name.endsWith(".dsi")||file.name.endsWith(".ids")){console.log("IS NDS file!");context.pushFiles([file]);/*            var reader = new FileReader();
...
}});}static get properties(){return{saving:{type:Boolean,notify:!0,value:!1},cardList:{type:Array,notify:!0},ndsFiles:{type:Array,notify:!0,value:null}}}}window.customElements.define("game-list",GameList)});